...
2. Fix templates/cron/01-core-infra.cron: replace hardcoded `/home/aldo/dev/01-core-infra/` and `aldo` with `__HOME__`/`__USER__`/`__CORE_INFRA__`; remove stray `\x07` bell characters.
...
- [ ] 1. Fix .husky/pre-commit — set $TEMPLATE variable, fix quoting, add cron template path
  ...
  QA scenarios:
    ...
    - Failure: `bash .husky/pre-commit` with a cron template containing `/home/aldo/dev/01-core-infra` — exits 1 with "Detected hardcoded path" message.
...
- [ ] 2. Fix templates/cron/01-core-infra.cron — replace hardcoded paths with placeholders, remove bell chars
  What to do / Must NOT do:
    ...
    - Replace `/home/aldo/dev/01-core-infra` with `__CORE_INFRA__` on both cron lines.
  ...
  Acceptance criteria (agent-executable): `grep -q '__HOME__' templates/cron/01-core-infra.cron` exits 0; `grep -q '__USER__' templates/cron/01-core-infra.cron` exits 0; `grep -q '__CORE_INFRA__' templates/cron/01-core-infra.cron` exits 0; `grep -q '/home/aldo/dev/01-core-infra' templates/cron/01-core-infra.cron` exits 1 (rejected); `grep -q $'\x07' templates/cron/01-core-infra.cron` exits 1 (no bell chars).
  QA scenarios:
